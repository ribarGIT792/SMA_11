...
maxnint=4; % kiek kartu didinti integravimo tasku skaiciu
 
for iii=1:maxnint

n=nint*2^(iii-1)+1     % integravimo tasku skaicius
...
ff=subs(f,sym(x),sym(xxx))
figure(iii),hold on, grid on, plot(xxx,ff, 'Linewidth',2);
title(sprintf('N=%d',N));
...
Integralas(1:maxnint)
% Integralo reik?m?s tikslinimas Rombergo metodu:
spalvos={'b*','m*','c*','r*','y*','k*','k*','k*','k*'};
if floor(N/2)*2 == N, NN=N, else NN=N+1; end  %metodo tikslumo eile
NN
 for i=1:maxnint-1
     for j=1:maxnint-1
         Integralas(j)=(2^(i+NN-1)*Integralas(j+1)-Integralas(j))/(2^(i+NN-1)-1);
     end
     i
     Integralas(1:maxnint-i)
%      plot([i+1:maxnint],Integralas(1:maxnint-i),spalvos{i});
 end

plot([maxnint],Integralas(1),'g*');

title (sprintf('Niutono-Koteso formule N=%d',N));
legend({'tiksli integralo reiksme','skaitiskai apskaiciuotos reiksmes prie skirtingu diskretizaciju', 'reiksme pagal Rombergo metoda' })
